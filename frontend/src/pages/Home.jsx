...
import { fetchCollections, fetchProducts } from "../shopify.js";
...
export default function Home() {
  ...
  useEffect(() => {
    fetchCollections().then(setCategories).catch(() => {});
    fetchProducts({ sort: "featured", first: 8 }).then(setFeatured).catch(() => {});
    fetchProducts({ category: "home-decor", first: 3 }).then(setHeroProducts).catch(() => {});
  }, []);

  const marquee = ["Free UK shipping over £25", "Secure Shopify checkout", "30-day easy returns", "New drops every month", "Trusted by thousands of homes"];
            ...
            {categories.map((c) => (
              <Link className="cat-card" to={`/shop?cat=${c.slug}`} key={c.slug}>
                ...
                <div className="cat-info">
                  <div><h3>{c.name}</h3><small>{c.tagline}</small></div>
